...
class SummaryEmulator(pl.LightningModule):
    def __init__(
        self,
...
class TpcfData(pl.LightningDataModule):
    def __init__(
        self,
        data_dir: str = "../../data/tpcf/z_0.50/",
        n_summary: int = 74,
    ...
    def prepare_data(self) -> None:
        ...
        normalized_train_parameters, normalized_train_summaries = normalize_parameters_and_summaries(
        ...
        normalized_val_parameters, normalized_val_summaries = normalize_parameters_and_summaries(
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        """
        return torch.utils.data.DataLoader(self.train_dataset, batch_size=256, num_workers=4, shuffle=True)
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        """
        return torch.utils.data.DataLoader(self.val_dataset, batch_size=64, num_workers=4, shuffle=True)
    ...
    def test_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        """
        return torch.utils.data.DataLoader(self.test_dataset, batch_size=64, num_workers=4, shuffle=True)


class PkData(pl.LightningDataModule):
    def __init__(
        self,
        data_dir: str = "../../data/pk/z_0.50/",
        n_summary: int = 1329,
        param_names: List[str] = ["Omega_m", "Omega_b", "h", "sigma_8", "n_s"],
    ):
        """
        Class to load the Quijote power spectra dataset, pre-process it, and prepare the train,
        validation and test datasets for the emulator
    ...
    def load_modes(self) -> torch.Tensor:
        """
        Loads the Fourier mode bins from a .nc file of the Quijote data

        Returns:
            modes (torch.Tensor): a Tensor of the mode bins

        Raises:
            FileNotFoundError: raises an exception if the data path is incorrect
        """
        # TODO: get the k array directly from the SummarizerDatasetLoader
        try:
            modes = torch.from_numpy(
                xr.open_dataarray(self.data_dir + "quijote_node0.nc").sel(multipoles=0).k.to_numpy()
            ).float()
            return modes
        except FileNotFoundError as err:
            raise FileNotFoundError(f"Error reading data file: {err}")

    def load_parameters_and_summaries(self, stage: str, k_crop: float = None) -> Tuple[torch.Tensor]:
        """
        Loads the cosmological parameters and the power spectrum for a given stage

        Args:
            stage (str): the stage for which to load the parameters and summaries (can be "train", "val" or "test")
            k_crop (float): the modes above which to crop the power spectrum. This feature may mostly be used to
                            avoid the small-scale spurious signal due to the Pylians CiC deconvolution feature

        Returns:
            Tuple:
                parameters (torch.Tensor): the cosmological parameters
                summaries (torch.Tensor): the summaries [P(k)]
        """
        loader = SummarizerDatasetLoader(
            stage=stage,
            data_dir=self.data_dir,
        ...
        pk = torch.from_numpy(loader.get_all_data()[:, :k_crop]).float()
        parameters = torch.from_numpy(loader.get_all_parameters()).float()
        summaries = pk
        return parameters, summaries
...
def get_mean_and_std(tensor: torch.Tensor) -> Tuple[torch.Tensor]:
    """
...
def normalize_parameters_and_summaries(parameters: torch.Tensor, summaries: torch.Tensor) -> Tuple[torch.Tensor]:
    """
    ...
    """
    parameters_mean, parameters_std = get_mean_and_std(parameters)
    summaries_mean, summaries_std = get_mean_and_std(summaries)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="QuickSummary",
        description="Training script for summary statistics emulator based on the Quijote simulations suite.",
    )
    parser.add_argument(
        "--n_epochs",
        type=int,
        required=True,
        help="Number of epochs required for the training (int)",
    )
    parser.add_argument(
        "--path_logs",
        default="logs",
        type=str,
        required=False,
        help="Path for the output logs (str)",
    )
    parser.add_argument(
        "--with-gpu",
        action="store_true",
        required=False,
        help="Accelerate the training with a GPU",
    )
    parser.add_argument(
        "--cosmo_params",
        nargs="+",
    ...
    # Data module
    data = PkData("../../data/pk/z_0.50/", param_names=args.cosmo_params)

    # Summary statistics emulator
    architecture = [args.size_hidden for i in range(args.number_hidden)]
    emulator = SummaryEmulator(
        data_module=data,
        n_input=len(args.cosmo_params),
        n_summary=args.n_bins,
        n_hidden=architecture,
    )
