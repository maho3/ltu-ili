...
def test_npe(monkeypatch):
    ...
    train_args = {"training_batch_size": 32, "learning_rate": 1e-4, "max_epochs": 5}
    ...
    nets += [
        ili.utils.load_nde_lampe(model=name, hidden_features=8, num_transforms=2)
    ...
    metric(posterior=posterior, x_obs=x[ind], theta_fid=theta[ind], x=x, theta=theta)
    ...
    proposal = prior
    prior = ili.utils.IndependentNormal(loc=torch.zeros(3), scale=0.1 * torch.ones(3))
    runner = LampeRunner(
        prior=prior,
        nets=nets[:2],
        engine=engine,
    ...
    # test TorchLoader
    train_dataset = TensorDataset(torch.Tensor(x[:-20]), torch.Tensor(theta[:-20]))
    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)

    val_dataset = TensorDataset(torch.Tensor(x[-20:]), torch.Tensor(theta[-20:]))
    val_loader = DataLoader(val_dataset, batch_size=32, shuffle=True)

    loader = ili.dataloaders.TorchLoader(train_loader, val_loader)
...
def test_zuko(monkeypatch):
    ...
    unittest.TestCase().assertRaises(ValueError, model, theta * 5, x)
...
def test_yaml():
    ...
    # Yaml file for data
    data = {"in_dir": "./toy_lampe", "x_file": "x.npy", "theta_file": "theta.npy"}
    ...
    data = {
        ...
        "metrics": {
            "single_example": {
                ...
                "args": {"num_samples": 20, "sample_method": "direct"},
            }
        },
    }
...
def test_universal(monkeypatch):
    ...
    # define training arguments
    nets = [
        ili.utils.load_nde_lampe(model="mdn", hidden_features=50, num_components=2),
    ]

    # check that the correct trainers are loaded
    runner = InferenceRunner.load(backend="lampe", engine="NPE", prior=prior, nets=nets)
