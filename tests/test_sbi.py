...
def test_snpe(monkeypatch):
    ...
    train_args = {"training_batch_size": 32, "learning_rate": 1e-4, "max_num_epochs": 5}
    ...
    fig = metric(
        posterior=posterior, x_obs=x[ind], theta_fid=theta[ind], x=x, theta=theta
    )
...
def test_snle(monkeypatch):
    ...
    for npar, simulator in zip([3, 1], [simulator_0, simulator_1]):
        ...
        nets = [
            ili.utils.load_nde_sbi(
                engine="NLE", model="maf", hidden_features=16, num_transforms=2
            ),
            ili.utils.load_nde_sbi(
                engine="NLE", model="made", hidden_features=16, num_transforms=2
            ),
        ]
        ...
        samples = (
            posterior.sample(
                (nsamples,), x[ind], method="slice_np_vectorized", num_chains=2
            )
            .detach()
            .cpu()
            .numpy()
        ...
        posterior.log_prob(samples, x[ind]).detach().cpu().numpy()
        ...
        metric = PlotSinglePosterior(
            ...
            num_samples=nsamples,
            sample_method="vi",
            sample_params={"dist": "maf", "n_particles": 32, "learning_rate": 0.01},
            labels=[f"$\\theta_{i}$" for i in range(npar)],
        )
        metric(
            posterior=posterior, x_obs=x[ind], theta_fid=theta[ind], x=x, theta=theta
...
def test_snre():
    ...
    nets = [
        ili.utils.load_nde_sbi(
            engine="NRE", model="resnet", hidden_features=16, num_blocks=3
        ),
...
def test_multiround():
    ...
    np.testing.assert_almost_equal(
        np.squeeze(all_loader[0].get_fid_parameters()), np.squeeze(theta0)
    )
...
def test_prior():
    ...
    for p in priors:
        ...
        nets = [
            ili.utils.load_nde_sbi(
                engine="NPE", model="maf", hidden_features=16, num_transforms=2
            )
        ]
...
def test_yaml():
    ...
    data = {"in_dir": "./toy", "x_file": "x.npy", "theta_file": "theta.npy"}
    with open("./toy/data.yml", "w") as outfile:
        yaml.dump(data, outfile, default_flow_style=False)

    # Yaml file for data - subset
    data = {"in_dir": "./toy", "x_file": "x_val.npy", "theta_file": "theta_val.npy"}
        ...
        "metrics": {
            ...
            "save_samples": {
                ...
                "args": {
                    "num_samples": 1,
                    "sample_method": "direct",
                    "sample_params": {"num_chains": 1, "burn_in": 1, "thin": 1},
                },
            },
        },
    }
...
def test_loaders():
    ...
    unittest.TestCase().assertRaises(ValueError, NumpyLoader, x, theta)
    ...
    # Check TorchLoader
    dataset = torch.utils.data.TensorDataset(torch.Tensor(x), torch.Tensor(theta))
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=32, shuffle=True)
    loader = TorchLoader(
        train_loader=dataloader,
        val_loader=dataloader,
        xobs=torch.Tensor(x[0]),
        thetafid=torch.Tensor(theta[0]),
    )
    ...
    loader = SBISimulator(
        in_dir="./toy", xobs_file="xobs.npy", num_simulations=10, simulator=simulator
    )
    ...
    for cat in all_cat:
        s = summary(cat)
        summary.store_summary(Path("./toy") / f"{summary!s}/{cat!s}.nc", s)
...
def test_universal():
    ...
    nets = [
        ili.utils.load_nde_sbi(
            engine="NPE", model="maf", hidden_features=50, num_transforms=5
        ),
        ili.utils.load_nde_sbi(
            engine="NPE", model="mdn", hidden_features=50, num_components=2
        ),
    ]
    ...
    unittest.TestCase().assertRaises(NotImplementedError, InferenceRunner)

    # check that the correct trainers are loaded
    runner0 = InferenceRunner.load(backend="sbi", engine="NPE", prior=prior, nets=nets)
    assert isinstance(runner0, SBIRunner)

    runner1 = InferenceRunner.load(backend="sbi", engine="SNPE", prior=prior, nets=nets)
    ...
    # test that it works
    load_nde_sbi(engine="NPE", model="maf", hidden_features=50, num_transforms=5)
    ...
    load_nde_sbi(engine="NLE", model="maf", hidden_features=50)
...
def test_misc():
    ...
    nets = [
        ...
        ili.utils.load_nde_sbi(
            engine="NPE", model="mdn", hidden_features=16, num_components=2
        ),
    ]
    ...
    unittest.TestCase().assertRaises(ValueError, runner, loader=loader)
