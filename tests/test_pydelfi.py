...
def test_toy():
    ...
    unittest.TestCase().assertRaises(
        ValueError, metric, posterior=posterior, x=x0, theta=theta0
    )
...
def test_prior():
    ...
    for p in priors:
        ...
        posterior = DelfiWrapper.load_engine("./toy_pydelfi/prior_posterior.pkl")
        # Check sampling of the DelfiWrapper
        theta0 = np.zeros(3) + 0.5
        x0 = simulator(theta0)
...
def test_custom_priors():
    ...
    testing.assert_allclose(
        prior.logpdf(loc), np.sum(norm.logpdf(loc, loc=loc, scale=scale))
    )
    testing.assert_allclose(
        prior.pdf(loc), np.prod(norm.pdf(loc, loc=loc, scale=scale))
    )
    ...
    low = np.zeros(3)
    high = np.ones(3)
    prior = IndependentTruncatedNormal(loc=loc, scale=scale, low=low, high=high)
    testing.assert_array_equal(prior.draw().shape, loc.shape)
...
def test_yaml():
    ...
    # Yaml file for data
    data = {"in_dir": "./toy_pydelfi", "x_file": "x.npy", "theta_file": "theta.npy"}
...
def test_universal():
    ...
    tf.reset_default_graph()
    load_nde_pydelfi(
        n_params=theta.shape[1], n_data=x.shape[1], model="maf", hidden_features=50
    )
