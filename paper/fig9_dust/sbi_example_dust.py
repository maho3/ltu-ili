...
ax1.plot(binLimsColour[:-1], x[:, : len(binLimsColour) - 1].T, color="k", alpha=0.1)
...
for i in np.arange(_N):
    ...
    for j in np.arange(3):
        dat[i, j, :] = np.percentile(posterior_samples[:, j], q=[15.9, 50, 84.1])
...
for i, (label, _x_o) in enumerate(
    ...
    corner.corner(
        posterior_samples,
        labels=["$\\rm \\tau_{v}^{ISM}$", "$\\rm \\tau_{v}^{BC}$", "$\\alpha$"],
        show_titles=True,
...
plt.legend(
    handles=[
        mpl.lines.Line2D([], [], color=f"C{i}", label=labels[i]) for i in range(3)
    ],
    fontsize=15,
    frameon=False,
    bbox_to_anchor=(1, 3),
