...
N_sims = (
    len(gamma_min_arr) * len(gamma_max_arr) * len(tau_V_ISM_arr) * len(tau_V_BC_arr)
)
    ...
    for j, gamma_max in enumerate(gamma_max_arr):
        for k, tau_V_ISM in enumerate(tau_V_ISM_arr):
            for l, tau_V_BC in enumerate(tau_V_BC_arr):
                index = (
                    i * len(gamma_max_arr) * len(tau_V_BC_arr) * len(tau_V_ISM_arr)
...
idx = 100
x_o = np.log10(calc_df(mags[idx, :, 2], (25.0 / h) ** 3, binLimits)[0])
x_o[np.isinf(x_o)] = -10
...
posterior_samples = posterior.sample((N_samps,), x=torch.Tensor(x_o).to("cuda"))
...
with np.errstate(divide="ignore"):
    mean = torch.tensor(np.median(np.array(posterior_samples), axis=0), device="cuda")
